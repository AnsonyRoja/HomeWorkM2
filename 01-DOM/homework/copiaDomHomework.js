...
let agregarNombre = document.querySelector('#createdBy');
agregarNombre.innerHTML +=  "Ansony";

// Crear una clase denominada 'npm teToDo' cuyo constructor debe recibir un único parámetro del tipo string
...
//  let arr = toDos.map(buildToDo);
 ...
 let arr = toDos.map((element,index)=> buildToDo(element,index));
...
function displayToDos() {
  ...
  let divs = buildToDos(toDoItems);

  divs.forEach((elements) => toDoContainer.appendChild(elements));
...
function addToDo() {
  ...
  if (input.value.trim() === "") return;
    ...
    // 1
    let tarea = new ToDo(input.value);
